### **Black-Scholes 選擇權定價公式（買權）**  
\[ C_0 = S_0 e^{-\delta T} N(d_1) - X e^{-rT} N(d_2) \]  

#### **變數說明**  

| 符號 | 變數名稱                | 解釋                                                                 | 影響方向（對買權價值） |
|------|-------------------------|----------------------------------------------------------------------|------------------------|
| \( C_0 \) | 買權現值               | 當前買權的理論價格（即你要付的權利金）。                            | —                      |
| \( S_0 \) | 標的資產當前價格       | 例如股票的現價。                                                    | ↑ 股價越高，買權越值錢 |
| \( X \)   | 執行價格（Exercise Price） | 未來可依此價格買入標的資產（如股票）。                             | ↑ 執行價越低，買權越值錢 |
| \( T \)   | 到期時間（年化）       | 從現在到選擇權到期的時間（以年為單位，例如 0.5 年 = 6 個月）。      | ↑ 時間越長，買權越值錢 |
| \( r \)   | 無風險利率             | 通常用國債利率代表，反映資金的時間價值。                            | ↑ 利率越高，買權越值錢 |
| \( \delta \) | 標的資產的股利收益率   | 股票等資產在持有期間發放的股利（例如股息率 2%）。                   | ↓ 股利越高，買權越不值錢 |
| \( \sigma \) | 標的資產的波動率       | 股價的年化標準差，代表價格波動程度（例如 30%）。                    | ↑ 波動越大，買權越值錢 |
| \( N(d) \) | 標準常態累積分布函數   | 計算 \( d_1 \) 和 \( d_2 \) 對應的累積機率，代表「價內」的可能性。 | —                      |

---

### **輔助變數 \( d_1 \) 和 \( d_2 \) 的意義**  
1. **\( d_1 \)**：  
   - 反映「標的資產價格上漲至價內的調整後機率」，考慮了股價、執行價、利率、股利和波動率。  
   - 公式：  
     \[ d_1 = \frac{\ln(S_0 / X) + (r - \delta + \sigma^2 / 2)T}{\sigma \sqrt{T}} \]  

2. **\( d_2 \)**：  
   - 本質上是 \( d_1 \) 扣除波動率的影響，用於計算「到期時標的資產價格 ≥ 執行價」的風險中性機率。  
   - 公式：  
...
### **經濟直覺**  
- **第一項 \( S_0 e^{-\delta T} N(d_1) \)**：  
  代表「預期標的資產未來價格」的現值，乘上「價內機率」。  
- **第二項 \( X e^{-rT} N(d_2) \)**：  
  代表「執行價的現值」，乘上「到期時選擇權被執行的機率」。  
- **整體公式**：  
  買權價值 = 預期資產收益的現值 － 執行成本的現值。  

---

### **舉例計算（假設參數）**  
若台積電股票：  
- \( S_0 = 600 \) 元，\( X = 550 \) 元，\( T = 0.5 \) 年（6個月），  
- \( r = 3\% \)，\( \delta = 2\% \)，\( \sigma = 30\% \)。  

1. 計算 \( d_1 \) 和 \( d_2 \)：  
   \[ d_1 = \frac{\ln(600/550) + (0.03 - 0.02 + 0.3^2 / 2) \times 0.5}{0.3 \times \sqrt{0.5}} \approx 0.48 \]  
   \[ d_2 = 0.48 - 0.3 \times \sqrt{0.5} \approx 0.27 \]  

2. 查標準常態表：  
   \( N(d_1) \approx 0.684 \)，\( N(d_2) \approx 0.606 \)。  

3. 代入公式：  
   \[ C_0 = 600 \times e^{-0.02 \times 0.5} \times 0.684 - 550 \times e^{-0.03 \times 0.5} \times 0.606 \approx 68.4 \text{ 元} \]  
   → 此買權的理論價格約 **68.4 元**。  

---

### **為什麼股利（δ）會降低買權價值？**  
- 股利發放會使股價下跌（除息效應），因此買權持有人「未來高價買入」的優勢減少。  
- 公式中 \( e^{-\delta T} \) 直接降低標的資產的預期貢獻。  

---

### **總結**  
Black-Scholes 公式將選擇權價值拆解為：  
1. **資產增值潛力**（\( S_0 \) 和 \( N(d_1) \)）。  
2. **執行成本現值**（\( X \) 和 \( N(d_2) \)）。  
所有
