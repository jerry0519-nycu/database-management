| 變數變化（Mundell-Fleming） | 匯率制度 | IS-LM（開放經濟）效果                          | 對 AD 的影響     | AS 是否變動？ | 原因與說明                  |
| --------------------- | ---- | -------------------------------------- | ------------ | -------- | ---------------------- |
| 🔺擴張性財政政策（G↑、T↓）      | 固定匯率 | IS 右移 → E↑ → NX↓（但中央銀行出手維持匯率）→ LM 右移支撐 | **AD 右移**    | 不變（短期）   | 因 LM 被動配合 → Y 上升，AD 增加 |
| 🔺擴張性財政政策（G↑、T↓）      | 浮動匯率 | IS 右移 → E↑ → NX↓ → IS 被抵消              | **AD 幾乎不變**  | 不變       | 匯率升值抵銷效果，AD 不明顯動       |
| 🔺擴張性貨幣政策（M↑）         | 浮動匯率 | LM 右移 → r↓ → E↓ → NX↑ → IS 右移          | **AD 右移（強）** | 不變       | Y 增加，物價可能上升，AD 明顯右移    |
| 🔺擴張性貨幣政策（M↑）         | 固定匯率 | 中央銀行會干預以維持匯率，**M 無法擴張**                | **AD 不變**    | 不變       | 貨幣政策無效，Y 不變，AD 也不動     |
| 🔺外國需求上升（NX↑）         | 任意匯率 | IS 右移                                  | **AD 右移**    | 不變       | 出口帶動總需求上升，Y 增加         |
| 🔻資本外逃（資金流出）          | 固定匯率 | r 有上升壓力 → 中央銀行售外匯，減少 M → LM 左移         | **AD 左移**    | 不變       | M 減少造成利率上升、Y 下降，AD 減少  |
| 🔺投資者對國內信心上升（資金流入）    | 固定匯率 | 中央銀行買進外匯 → M↑ → LM 右移                  | **AD 右移**    | 不變       | 自動刺激效果                 |
