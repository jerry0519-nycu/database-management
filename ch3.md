...
| Join 類型         | 執行流程                                                                 | 結果分析                                                                 | 對同一張表操作的結果範例 |
|-------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------|
| **自然連接(NATURAL JOIN)** | 1. 自動匹配兩表同名列<br>2. 只保留匹配成功的行<br>3. 合併同名列為單一列 | 只顯示兩表有匹配的記錄<br>結果行數 ≤ 任一原表行數 | 若對`員工`自連接(匹配`dept_id`):<br>結果只有有部門的員工(張三、李四) |
| **左外連接(LEFT OUTER JOIN)** | 1. 保留左表所有行<br>2. 匹配右表記錄<br>3. 無匹配則右表欄位填NULL | 結果行數 = 左表行數<br>包含左表所有記錄 | 員工表左連接自身:<br>包含所有員工(王五也會出現，右側為NULL) |
| **右外連接(RIGHT OUTER JOIN)** | 1. 保留右表所有行<br>2. 匹配左表記錄<br>3. 無匹配則左表欄位填NULL | 結果行數 = 右表行數<br>包含右表所有記錄 | 員工表右連接自身:<br>包含所有員工(王五也會出現，左側為NULL) |

## 詳細分析

### 1. 自然連接(NATURAL JOIN)分析
```sql
...
**特點**:
- 王五因`dept_id`為NULL被排除
- 技術部(103)因無員工也被排除
- 合併後的`dept_id`只出現一次
  

#### 按照product->select->project流程
##### 1. 乘積(Cartesian Product)
先執行兩表的笛卡兒積：
...
##### 2. 選擇(Selection)
應用自然連接的條件：`e.dept_id = d.dept_id` (注意NULL的處理)

**重要**：在SQL中，任何與NULL的比較都會返回UNKNOWN，不會被選中
...
<details> <summary><strong>運算子操作比較</strong></summary>
